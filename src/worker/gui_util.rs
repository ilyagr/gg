...
impl WorkspaceSession<'_> {
    ...
    /// Like disinherit_children, but skips children whose commit IDs are in the exclusion set.
    /// Used when moving a range of commits: we disinherit external children of the newest commit
    /// but keep the internal structure of the range intact.
    pub async fn disinherit_children_excluding(
        &self,
        tx: &mut Transaction,
        target: &Commit,
        exclude: &HashSet<CommitId>,
    ) -> Result<HashMap<CommitId, CommitId>> {
        // find all children of target, excluding those in the set
        let children_expr = RevsetExpression::commit(target.id().clone()).children();
        let children: Vec<_> = children_expr
            .evaluate(self.operation.repo.as_ref())?
            .iter()
        ...
        let external_children: Vec<_> = children
            .into_iter()
        ...
        // rebase each external child, replacing target with target's parents
        let mut rebased_commit_ids = HashMap::new();
