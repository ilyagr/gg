...
impl Mutation for BackoutRevisions {
    async fn execute(self: Box<Self>, ws: &mut WorkspaceSession) -> Result<MutationResult> {
        ...
        // newest = first in list (descendants first), oldest = last
        let newest = commits.first().ok_or_else(|| anyhow!("empty revset"))?;
        let oldest = commits.last().ok_or_else(|| anyhow!("empty revset"))?;

        // get parents of oldest commit as the base before all changes
        let oldest_parents: Result<Vec<_>, BackendError> = oldest.parents().collect();
        let oldest_parents = oldest_parents?;

        // 3-way merge to reverse the changes
        let old_base_tree = rewrite::merge_commit_trees(tx.repo(), &oldest_parents).await?;
        let new_base_tree = working_copy.tree();
        let old_tree = newest.tree();

        let (reverted_label, parents_label) = if commits.len() == 1 {
            (
        ...
        let new_tree = MergedTree::merge(Merge::from_vec(vec![
            ...
            (old_tree, reverted_label),
            (old_base_tree, parents_label),
        ]))
        .await?;
...
impl Mutation for MoveRevisions {
    async fn execute(self: Box<Self>, ws: &mut WorkspaceSession) -> Result<MutationResult> {
        ...
        // rebase the oldest commit onto new parents; the rest of the range follows
        let transaction_description = if targets.len() == 1 {
